        ...
        <div class="card shadow mb-4">
            <div class="card-header py-3">
                <div class="row no-gutters">
                    <div class="col md-10">
                        <h1 class="h4 mb-0 font-weight-bold text-primary" style="margin-top: 0.2em;">My Event List</h1>
                    </div>
                    <div class="col md-2">
                        ...
                        @include('creator.event.crud.create')
                    </div>
                </div>
            </div>
            <div class="card-body">
                    ...
                    @if(count($events) > 0)
                        <table class="table table-bordered" id="table" width="100%" cellspacing="0">
                            ...
                            @foreach($events as $event)
                                <tr class="text-center">
                                    <td>{{$event->id}}</td>
                                    <td><a href="{{ route('creator.events.show', $event->id) }}">{{$event->title}}</a></td>
                                    <td><a type="button" class="text-primary" data-toggle="modal"
                                           data-target="#showModal-{{$event->id}}">Read</a></td>
                                    @include('creator.event.show_description')
                                    <td>{{$event->noa}}</td>
                                    ...
                                    <td width="150px">
                                        <div class="row no-gutters">
                                            ...
                                            <div class="col-md-6">
                                                <form action="{{route('creator.events.destroy', $event->id)}}" method="POST">
                                                    {{ csrf_field() }}
